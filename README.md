...
- **公体下蛋拦截（可配）**：鸵鸟/食火鸡开启 `BlockEggLaying` 后，公体不再产蛋，母体仍正常下蛋（符合生物学）
...
4. **准星对准**任意可繁殖生物时，信息随准星显示：
   - **已安装 Neorxna**：NeorxnaIE 同款机制——把性别（♂公亮蓝 / ♀母粉）、成长阶段、成长值百分比、繁殖状态、成长进度条，**塞进 Neorxna 现成的顶部信息面板 NUInfoPanelWidget**（不另画 UI）。
   - **未安装 Neorxna**：回退到模组自带面板（名称+性别、血量条、成长值 + 成长进度条、繁殖状态）。
5. 打开图鉴可查看每种生物的详细介绍与繁殖属性。
...
- **模组设置**：游戏内 **设置 → 模组设置 → 动物繁殖系统显示设置** 实时生效（`MOD/modsettings.json`），包含三大区块：
  - **头顶悬浮文字**：开关、字号缩放（0.5~3×）、上下偏移（避免大模型遮挡）
  - **元素开关**：名称 / 性别 / 成长阶段 / 繁殖状态 / 成长进度条 各自独立开关
  - **Neorxna 面板注入**：总开关 + 性别 / 阶段 / 状态 / 成长进度条（装 Neorxna 时生效）
...
├── MOD/
│   ├── modinfo.json               # 模组元信息
│   ├── modsettings.json           # 模组设置（显示设置：悬浮文字/元素开关/Neorxna）
│   └── Assets/
│       ├── BreedingConfig.json    # 运行配置（灰狼块含注释模板）
│       └── Lang/                  # 7 种语言翻译（设置 + 图鉴介绍）
